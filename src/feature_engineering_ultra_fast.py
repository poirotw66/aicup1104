"""
Feature Engineering Module - ULTRA FAST VERSION
移除 groupby().apply() 中的 lambda，實現完全向量化
預期比 fast 版本再快 2-3 倍
"""
...
def extract_features_ultra_fast(
    transaction_loader,
    alert_accounts: Set[str],
    save_path: str = 'output/features.csv'
) -> pd.DataFrame:
    """
    超級優化版本 - 避免 groupby().apply() 中的 lambda
    ...
    print("完全向量化 - 避免 groupby().apply() 瓶頸")
    print("="*60)
    ...
    print(f"合併 {len(all_chunks)} 個批次...")
    txn_df = pd.concat(all_chunks, ignore_index=True)
    del all_chunks
    gc.collect()
    
    print(f"總交易數: {len(txn_df):,}")
    ...
    out_features = txn_df.groupby('from_acct').agg({
        ...
        'is_unk': 'sum',
        'is_alert_out': 'sum',
        'from_acct_type': 'nunique',
        'channel_type': 'nunique',
        # 時間特徵（完全向量化，無 lambda！）
        'is_night': 'sum',
        'is_work_hour': 'sum',
        'hour': 'sum',  # 後面會除以 valid_hour 數量
        'valid_hour': 'sum'
    }).reset_index()
    ...
    out_features.columns = [
        ...
        'out_night_sum', 'out_work_sum', 'out_hour_sum', 'out_valid_hour_count'
    ]
    
    # 計算時間比例（向量化）
    out_features['out_night_ratio'] = out_features['out_night_sum'] / out_features['out_txn_count']
    out_features['out_work_ratio'] = out_features['out_work_sum'] / out_features['out_txn_count']
    out_features['out_avg_hour'] = np.where(
        out_features['out_valid_hour_count'] > 0,
        out_features['out_hour_sum'] / out_features['out_valid_hour_count'],
        0
    )
    
    # 刪除中間欄位
    out_features = out_features.drop(['out_night_sum', 'out_work_sum', 'out_hour_sum', 'out_valid_hour_count'], axis=1)
    ...
    print(f"✓ 轉出特徵: {len(out_features):,} 個帳戶")
    ...
    in_features.columns = [
        ...
        'in_night_sum', 'in_work_sum', 'in_hour_sum', 'in_valid_hour_count'
    ]
    
    # 計算時間比例
    in_features['in_night_ratio'] = in_features['in_night_sum'] / in_features['in_txn_count']
    in_features['in_work_ratio'] = in_features['in_work_sum'] / in_features['in_txn_count']
    in_features['in_avg_hour'] = np.where(
        in_features['in_valid_hour_count'] > 0,
        in_features['in_hour_sum'] / in_features['in_valid_hour_count'],
        0
    )
    
    in_features = in_features.drop(['in_night_sum', 'in_work_sum', 'in_hour_sum', 'in_valid_hour_count'], axis=1)
    ...
    print(f"✓ 轉入特徵: {len(in_features):,} 個帳戶")
    ...
    print("⚡⚡⚡ Ultra Fast 特徵提取完成！⚡⚡⚡")
    
    return features_df


if __name__ == "__main__":
    print("Ultra fast feature engineering module ready!")
